...
func spawn_fragment():
	if not loot_data.contains_unique_code_fragment:
		print("Baú não está configurado para conter fragmentos únicos.")
		return

	var unique_fragment: CodeFragmentData = LootManager.get_unique_code_fragment()
	if not unique_fragment:
		print("LootManager não tem mais fragmentos únicos disponíveis.")
		return

	var fragment_instance = CODE_FRAGMENT_SCENE.instantiate()
	fragment_instance.fragment_data = unique_fragment
	_spawn_item_with_animation(fragment_instance)
