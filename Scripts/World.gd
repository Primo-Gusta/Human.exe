...
func _ready() -> void:
	...
	# Pequeno atraso para evitar inputs residuais ao iniciar
...
func start_game() -> void:
	...
	if current_game_menus_instance == null:
		...
		# Garantir que receba input mesmo com a árvore pausada
		current_game_menus_instance.process_mode = Node.PROCESS_MODE_ALWAYS
		# Conectar sinal (se o script o emitir)
	...
	hud.visible = true
	# AQUI: Se a HUD já recebeu o max_health no _ready do World,
	# então precisamos apenas garantir que o valor atual seja exibido.
	if is_instance_valid(player) and is_instance_valid(hud):
		hud.update_health(player.health) # <--- ADICIONE ESTA LINHA AQUI
		hud.set_max_health(player.max_health) # <--- Se não foi feito no _ready do World, faça aqui também.
											  #      Mas idealmente set_max_health no _ready do World já é suficiente.
...
func _unhandled_input(event) -> void:
	# Ignora até o jogo estar pronto (evita abrir menus logo no load)
	if not game_ready_for_input:
		return

	# Se o jogo NÃO estiver pausado e nenhum menu visível, o World pode abrir menus
	if not get_tree().paused and \
	   ...
	   (current_pause_menu_instance == null or not current_pause_menu_instance.visible):
		...
		if event.is_action_pressed("toggle_pause"):
			# Só considerar o início do pressionamento (ignora echoes)
			if event.is_pressed() and not event.is_echo():
				get_viewport().set_input_as_handled()
				toggle_pause_menu()

		# Toggle GameMenus (Tab)
		elif event.is_action_pressed("toggle_game_menus"):
			if event.is_pressed() and not event.is_echo():
				get_viewport().set_input_as_handled()
				toggle_game_menus()

	# Se a árvore estiver pausada, os menus (process_mode = ALWAYS) tratarão seus próprios inputs.
...
func toggle_game_menus() -> void:
	...
	# Pausa a árvore; os menus continuarão processando por process_mode
...
func _on_game_menus_closed() -> void:
	...
	# Assume que GameMenus já chamou set_menu_state(false)
	get_tree().paused = false
	if hud:
		hud.visible = true
	Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED)
...
func _on_pause_menu_quit_to_main_menu_requested() -> void:
	...
	# Garantir que não fique pausado ao mudar de tela
	get_tree().paused = false

	# Caminho do MainMenu - ajuste se o arquivo estiver em outro local
